    ...
    return (
        <div className={styles.chart}>
            ...
            <div className={styles.innerbox}>
                {loading ? <div style={{'height': '20vh'}}></div> : Clicker.map((item, key) => <ul style={{ 'margin': '0px' }}><button onClick={() => handleClick(key)}>{item}</button></ul>)}
            </div>
            {chart[0] && !('error' in chart[0]) ? <div className={styles.chart_visual}>
                <p className={styles.header}>Aktienkurs</p>
            ...
            </div> : <p className={styles.error}>{loading?'':'Nothing to show'}</p>}
        </div>
    )
}